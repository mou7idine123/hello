    ...
    const getStatusBadge = (status) => {
        ...
        switch (status) {
            case 'En attente de virement':
                return <span className="badge badge-open" style={{ backgroundColor: '#fff3cd', color: '#856404' }}><Clock size={12} className="inline mr-1" /> {transStatus}</span>;
            case 'Reçu soumis':
                return <span className="badge badge-in-progress" style={{ backgroundColor: '#cce5ff', color: '#004085' }}><Upload size={12} className="inline mr-1" /> {transStatus}</span>;
            case 'Vérifié':
                                    ...
                                    <div key={don.id} style={{ border: '1px solid var(--border)', borderRadius: '0.5rem', overflow: 'hidden' }}>
                                        ...
                                        <div style={{ padding: '1.25rem' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', alignItems: 'center' }}>
                                                ...
                                                <span style={{ color: 'var(--primary)', fontWeight: 600 }}>{don.amount.toLocaleString()} {t('dashboard.mru')}</span>
                                            </div>
